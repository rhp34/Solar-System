...
void main()
{
    // --- a quick-and-dirty way for creating the light dummy
    if(u_shading==-1) {        
    ...
    mat3 tranTBN = transpose(TBN);

    // Transform light and view vector to TS
    v_lightTS = tranTBN * u_lightpos-v_pos;
    v_eyeTS  = tranTBN * v_pos;
...
{    
    ...
    else if(u_shading==2)
    {
        ...
        L = v_lightTS;
        V = -v_eyeTS;
        N = mix(vec3(0,0,1),(texture2D(u_nmSampler, v_uv).xyz * 2.0 - 1.0),u_hasSampler[2]);
    ...
    // Sample texture
    vec3 text2D = texture2D(u_kdSampler, v_uv).xyz;

    // Decide kd-value
    vec3 kdColor = mix(u_kd, text2D, u_hasSampler[0]);     
