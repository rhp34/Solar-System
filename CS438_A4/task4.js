...
function main() {
    ...
    const objectManager = new function () {
        ...
        let headmesh2 = new GeometryObject(importObj(head2));         
        ...
        // Register the event for face culling
        ...
        // Register the event for face winding        
        document.getElementById("frontface").onchange = function (event) {
            if(event.target.value=='ccw') {                        
                gl.frontFace(gl.CCW);
    ...
    const materialManager = new function () {
        ...
        const materials = {
            ...
            "Face": {
                ...
                qs: 50,
                diffSampler: loadTexture(gl, './texture/head2_diffuse.jpg'),
                normSampler: loadTexture(gl, './texture/head2_normal.jpg'),
            },
            "Earth": {
                colors: { "ka": [0.01, 0.01, 0.01], "kd": [0.5, 0.5, 0.5], "ks": [0.08, 0.08, 0.08] },
                qs: 50,
                diffSampler: loadTexture(gl, './texture/earth_diffuse.jpeg'),
                normSampler: loadTexture(gl, './texture/earth_normal.jpeg'),
            },

            "Sun": {
                colors: { "ka": [0.01, 0.01, 0.01], "kd": [0.5, 0.5, 0.5], "ks": [0.08, 0.08, 0.08] },
                qs: 50,
                diffSampler: loadTexture(gl, './texture/sun_diffuse.jpeg'),
                normSampler: loadTexture(gl, './texture/sun_normal.jpeg'),
            },

            "Moon": {
                colors: { "ka": [0.01, 0.01, 0.01], "kd": [0.5, 0.5, 0.5], "ks": [0.08, 0.08, 0.08] },
                qs: 50,
                diffSampler: loadTexture(gl, './texture/moon_diffuse.jpeg'),
                normSampler: loadTexture(gl, './texture/moon_normal.jpeg'),
            },
                // ************************************************************************
                // *** TODO_A4 : Task 4 *** 
                // 
